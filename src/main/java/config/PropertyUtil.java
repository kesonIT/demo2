...
public class PropertyUtil {
	private final AILogger logger = AILogger.getLogger(PropertyUtil.class);
	...
	public PropertyUtil(final String bundleName) {
		...
		this.logger.debug("PropertyUtil: bundleName={%s}", bundleName);
	}
	 ...
	 * @return the string matching the key
	 */
	public String getSystemOrPropertyString(final String key,
			final String defaultValue, final boolean useCache) {
		// if they want to use the cache, and the cache contains this key,
		// return the value
		if (useCache && this.cache.containsKey(key)) {
			return this.cache.get(key);
		}

		// Try to get the property from the system configuration, for example,
		// from, 'java
		// -Dkey=value'
		String result = System.getProperty(key);

		if (result == null) {
			// If we were unsuccessful in the cache and the system properties,
			// try to fetch from
		...
		// Do not create anything in the cache if they show an intention not to
		// use the cache
		...
		if (this.logger.isTraceEnabled()) {
			this.logger.debug(
					"Returning property value: <{%s}>=<{%s}> (default=<{%s}>)",
	...
	public void setPropertyBundleName(final String newPropertyBundleName) {
		Objects.requireNonNull(newPropertyBundleName,
				"Property bundle name cannot be null");
		this.bundleName = newPropertyBundleName;

		this.logger.debug("PropertyUtil: change bundleName={%s}", bundleName);
			...
			synchronized (this) {
				...
				if (result == null) {
					...
					if (userDir != null) {
						...
						this.logger
								.debug("Looking for property bundle in user.dir + subdirectory: {%s}",
										fullUserDir);
						final Path userDirPath = Paths.get(fullUserDir);
						if (Files.exists(userDirPath)) {
							try {
								final URL[] urls = { userDirPath.toUri()
										.toURL() };
								...
								this.logger
										.debug("Found property bundle in user.dir + subdirectory: {%s} {%s}",
												userDirPath.toUri().toURL(),
												this.bundleName);
							} catch (final MalformedURLException e) {
								this.logger.debug(
										"URL exception with user.dir:", e);
							} catch (final MissingResourceException e) {
								// Do nothing, will try other options
								this.logger
										.debug("Missing resource exception with user.dir:",
												e);
							}
						}
					} else {
						this.logger.debug("Could not find user.dir property");
					}
				}
				if (result == null) {
					...
					if (userHome != null) {
						...
						this.logger
								.debug("Looking for property bundle in user.home + subdirectory: {%s}",
										fullUserHome);
						final Path userHomePath = Paths.get(fullUserHome);
						if (Files.exists(userHomePath)) {
							try {
								final URL[] urls = { userHomePath.toUri()
										.toURL() };
								final ClassLoader loader = new URLClassLoader(
										urls);
								result = ResourceBundle
										.getBundle(
												...
												Control.getNoFallbackControl(Control.FORMAT_PROPERTIES));
								this.logger
										.debug("Found property bundle in user.home + subdirectory: {%s} {%s}",
												userHomePath.toUri().toURL(),
												this.bundleName);
							} catch (final MalformedURLException e) {
								this.logger.debug(
										"URL exception with user.home:", e);
							} catch (final MissingResourceException e) {
								// Do nothing, will try other options
								this.logger
										.debug("Missing resource exception with user.home:",
												e);
							}
						}
					} else {
						this.logger.debug("Could not find user.home property");
					}
				}
				if (result == null) {
					try {
						this.logger
								.debug("Looking for property bundle on classpath");
						// Try to resolve bundle on classpath
						result = ResourceBundle.getBundle(this.bundleName);
						this.logger.debug(
								"Found property bundle in classpath: {%s}",
								this.bundleName);
				...
				if (result == null) {
					this.logger.info("Could not find property bundle: {%s}",
							this.bundleName);
				}
				this.bundle = result;
			}
